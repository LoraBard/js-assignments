 ...
 *
 */
function getRectangleString(x, y) {
...
function encodeToRot13(str) {
  let k=0;
  var input= 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',output= 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm',newStr='';
  for(let i=0;i<str.length;++i){
      k=input.indexOf(str[i]);
      if(k!==-1){
          newStr+=output[k];
      }
      else{
...
function getCardId(value) {
 var symbols='A234567891JQK';
 if(value[1]=='♣'||value[2]=='♣'){
     return symbols.indexOf(value[0]);
 }
 if(value[1]=='♦'||value[2]=='♦'){
    return symbols.indexOf(value[0])+13;
 }
 if(value[1]=='♥'||value[2]=='♥'){
    return symbols.indexOf(value[0])+13*2;
 }
 if(value[1]=='♠'||value[2]=='♠'){
    return symbols.indexOf(value[0])+13*3;
 }
}
